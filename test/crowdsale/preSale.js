...
describe('CrybCrowdsale: preSale', () => {
  ...
  it('should revert if sold out', async () => {
    ...
    await crybCrowdsale.connect(participants[0]).buy({value: toBase(20)}) // 20 ETH will buy 200 tokens
    await crybCrowdsale.connect(participants[1]).buy({value: toBase(10)}) // 10 ETH will buy 100 tokens
    await crybCrowdsale.connect(participants[2]).buy({value: toBase(10)}) // 10 ETH will buy 100 tokens
  ...
  it('should revert if personal max allocation is reached', async () => {
    ...
    await crybCrowdsale.connect(participants[0]).buy({value: toBase(10)}) // 10 ETH will buy 100 tokens
    await crybCrowdsale.connect(participants[0]).buy({value: toBase(5)}) // 5 ETH will buy 50 tokens
    await crybCrowdsale.connect(participants[0]).buy({value: toBase(5)}) // 5 ETH will buy 50 tokens
  ...
  it('should increase the totalRaised value', async () => {
    await moveToPresaleStartTime()
    await crybCrowdsale.connect(participants[0]).buy({value: toBase(20)})
    let totalRaised = await crybCrowdsale.totalRaised()
    expect(totalRaised).to.equal(toBase(20))

    await crybCrowdsale.connect(participants[1]).buy({value: toBase(10)})
    totalRaised = await crybCrowdsale.totalRaised()
    expect(totalRaised).to.equal(toBase(30))

    await crybCrowdsale.connect(participants[2]).buy({value: toBase(10)})
    totalRaised = await crybCrowdsale.totalRaised()
  ...
  it('should not transfer the tokens to the buyer', async () => {
    ...
    await crybCrowdsale.connect(participants[1]).buy({value: toBase(15)})
  ...
  it('should create a new vesting position for the given users', async () => {
    ...
    let {blockNumber} = await crybCrowdsale.connect(participants[0]).buy({value: toBase(20)})
    ...
    ({blockNumber} = await crybCrowdsale.connect(participants[1]).buy({value: toBase(10)}));
  ...
  it('should allow the same user have multiple vesting positions', async () => {
    ...
    let {blockNumber} = await crybCrowdsale.connect(participants[0]).buy({value: toBase(10)})
    let {timestamp} = await ethers.provider.getBlock(blockNumber)
    let vestingInfo = await crybCrowdsale.getVestingInfo(participants[0].address, 0)
    expect(vestingInfo.amount).to.equal(toBase(100))
    expect(vestingInfo.totalClaimed).to.equal(0)
    ...
    // second position
    ({blockNumber} = await crybCrowdsale.connect(participants[0]).buy({value: toBase(5)}));
    ({timestamp} = await ethers.provider.getBlock(blockNumber));
  ...
  it('should update the vesting count for each user', async () => {
    ...
    await crybCrowdsale.connect(participants[0]).buy({value: toBase(5)})
  ...
  it('should emit Buy event', async () => {
    ...
    await expect(
      crybCrowdsale.connect(participants[0]).buy({value: toBase(10)})
    )
    .to
